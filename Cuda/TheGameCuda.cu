...
int main(void) {
    ...
    */
    ...
    int turnCount = 0;
    int* turnList = determineTurns(field, xDim, yDim, WHITE, &turnCount);
    int player = WHITE;

    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < turnCount; j++) {
            int count;
            int* turns = determineTurns(getFieldFromList(list, j, xDim, yDim), xDim, yDim, player, &count);
            
            for (int k = 0; k < count; k++) {
                addFieldToList(list, turnCount, getFieldFromList(turns, k, xDim, yDim), xDim, yDim);
                turnCount++;
            }
        }
        player = -player;
                ...
                for (int dir = -1; dir <= 1; dir++) {
                    ...
                    if (xNew >= 0 && xNew < xDim && yNew >= 0 && yNew < yDim) {
                        ...
                        if (!moveAllowed) {
                            ...
                            list = (int*) realloc(list, i * xDim * yDim * sizeof(int));
...
int* getFieldFromList(int* list, int index, int xDim, int yDim) {
    list + xDim * yDim * index;
}

int* addFieldToList(int* list, int listSize, int* field, int xDim, int yDim) {
    int* newList = (int*) realloc((listSize + 1) * xDim * yDim * sizeof(int));
